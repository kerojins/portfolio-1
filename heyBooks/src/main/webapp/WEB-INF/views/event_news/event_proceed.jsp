...
<div id="event_box">
	<div class="container">
		...
		<ul class="nav nav-tabs event_tabs">
			<li class="nav-item">
				<a class="nav-link active" href="<c:url value='event_proceed'/>">진행 중인 이벤트</a>
			</li>
			<li class="nav-item">
				<a class="nav-link" href="<c:url value='event_ended'/>">지난 이벤트</a>
			</li>
		</ul>
				...
				<li class="page-item">
					<c:choose>
						<c:when test="${util.pageNum < util.totalPageCount}">
							<a class="page-link"   href="<c:url value='/event_proceed?pageNum=${util.pageNum+1}'/>" aria-label="Next"> 
								<span aria-hidden="true">»</span>
							</a> 
						</c:when>   
						<c:otherwise>
