	...
	<div class="container overhidden">
			...
			<div class="aside_content">
				...
				<ul class="collapse aside_menu_list <c:if test='${cate_ref1 > 100  && cate_ref1 < 700}'>show</c:if>" id="collapseExample1">
					<c:forEach var="domestic_vo" items="${domestic_vo}">
						<li>
							<a <c:if test='${cate_num eq domestic_vo.cate_num}'>style="color:#0e7eda; font-weight:bold"</c:if> href='<c:url value="/weekly_best_seller?cate_num=${domestic_vo.cate_num}&cate_ref1=${domestic_vo.cate_ref1}"/>'>${domestic_vo.cate_name }</a>
						</li> 
					</c:forEach>  
				</ul>  
				<p>  
				...
				<ul class="collapse aside_menu_list <c:if test='${cate_ref1 >= 700}'>show</c:if>" id="collapseExample2">
					<c:forEach var="oversea_vo" items="${oversea_vo}"> 
						<li>
							<a <c:if test='${cate_num eq oversea_vo.cate_num}'>style="color:#0e7eda; font-weight:bold"</c:if> href='<c:url value="/weekly_best_seller?cate_num=${oversea_vo.cate_num }&cate_ref1=${oversea_vo.cate_ref1}"/>'>${oversea_vo.cate_name }</a>
						</li>
					</c:forEach>
				</ul> 
			</div>
		...
		<div class="sub_right_box best_seller_right">
			...
			<ul class="nav nav-tabs best_seller_nav">
				<li class="nav-item">
					<a class="nav-link" href="<c:url value='weekly_best_seller'/>">주간 베스트셀러</a>
				</li>
				<li class="nav-item">
					<a class="nav-link active" href="<c:url value='monthly_best_seller'/>">월간 베스트셀러</a>
				</li> 
			</ul>
			<div class="sub_right_content">
				<form class="best_seller_date_form" id="small_cate_form" method="post">
					<div class="itemList_top_menu"> 
						<div class="best_seller_date">
							...
							<select class="form-control best_year" name="best_year">
							</select> 
							<select class="form-control best_month" name="best_month">
							</select>  
							<input type="button" class="btn btn-primary best_search_btn" value="검색">
						</div> 
					...
					<div class="itemList_array_menu">
								...
								<c:choose> 
									<c:when test="${util.pageNum==i }"> 
										<li class="page-item active" aria-current="page">
											<a class="page-link" href="<c:url value='/weekly_best_seller?pageNum=${i}&year=${year}&month=${month}&week=${week}&cate_num=${cate_num}&cate_ref1=${cate_ref1}'/>"> ${(i-1)*20+1}위~${i*20}위 </a>
										</li> 
									</c:when> 
									<c:otherwise> 
										<li class="page-item"><a class="page-link" href="<c:url value='/weekly_best_seller?pageNum=${i}&year=${year}&month=${month}&week=${week}&cate_num=${cate_num}&cate_ref1=${cate_ref1}'/>">${(i-1)*20+1}위~${i*20}위 </a></li>
									</c:otherwise>
								</c:choose>    
							</c:forEach>   
						</ul>
						...
						<div class="best_seller_info">
							<p><span class="year_txt"></span>년도 <span class="month_txt"></span>월 베스트셀러</p>
						</div> 
					</div> 
