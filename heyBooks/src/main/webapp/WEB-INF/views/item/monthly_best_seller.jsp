			...
			<div class="aside_content">
				...
				<ul class="collapse aside_menu_list <c:if test='${cate_ref2 eq 100}'>show</c:if>" id="collapseExample1">
					<c:forEach var="domestic_vo" items="${domestic_vo}">
						<li><a <c:if test='${cate_num eq domestic_vo.cate_num}'>style="color:#0e7eda; font-weight:bold"</c:if> href='<c:url value="/monthly_best_seller?cate_num=${domestic_vo.cate_num}&cate_ref2=${domestic_vo.cate_ref2}"/>'>${domestic_vo.cate_name }</a></li>
					</c:forEach> 
				</ul> 
				<p>  
				...
				<ul class="collapse aside_menu_list <c:if test='${cate_ref2 eq 700}'>show</c:if>" id="collapseExample2">
					<c:forEach var="oversea_vo" items="${oversea_vo}"> 
						<li><a <c:if test='${cate_num eq oversea_vo.cate_num}'>style="color:#0e7eda; font-weight:bold"</c:if> href='<c:url value="/monthly_best_seller?cate_num=${oversea_vo.cate_num }&cate_ref2=${oversea_vo.cate_ref2}"/>'>${oversea_vo.cate_name }</a></li>
					</c:forEach>
				</ul> 
			</div>
		</aside>
